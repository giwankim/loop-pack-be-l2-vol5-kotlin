...
표기: 속성은 코드 이름, 규칙은 어기면 거절되는 조건, 행위는 공개 메서드다. 거절은 `CoreException(ErrorType, message)`로 나타내고 상태는 바꾸지 않는다.
...
- 이름은 앞뒤 공백을 뗀 뒤 비어 있지 않고 100자 이하다. 어기면 `INVALID_NAME`.
- 삭제되지 않은 브랜드끼리는 이름이 같을 수 없다. 이 규칙은 저장소를 봐야 하므로 브랜드 자신이 아니라 application이 등록·수정 전에 확인한다. 어기면 `BRAND_NAME_DUPLICATED`.
...
| 메서드 | 하는 일 | 거절 |
| --- | --- | --- |
| `Brand(name)` | 이름 규칙을 검사하고 만든다 | `INVALID_NAME` |
| `update(name)` | 이름을 바꾼다 | `INVALID_NAME` |
...
- 이름은 브랜드와 같은 규칙이다. 어기면 `INVALID_NAME`.
- 가격은 1원 이상 1,000,000,000원 이하다. `Money`가 음수를 막고 `Product`가 1원 이상과 상한을 막는다. 어기면 `INVALID_PRICE`.
...
| 메서드 | 하는 일 | 거절 |
| --- | --- | --- |
| `Product(brand, name, price, stock)` | 규칙을 검사하고 만든다 | `INVALID_NAME`, `INVALID_PRICE`, `INVALID_STOCK` |
| `update(name, price)` | 이름과 가격을 바꾼다. 하나라도 어기면 둘 다 그대로다 | `INVALID_NAME`, `INVALID_PRICE` |
| `updateStock(quantity)` | 재고를 최종 수량 `Stock(quantity)`로 바꾼다 | `INVALID_STOCK` |
...
- 고객 상세: `ProductService.getProduct` → 살아 있는 상품 조회(브랜드 포함) → 좋아요 수 조회 → `ProductInfo`. `soldOut`은 `product.isSoldOut()`에서 온다.
...
- 0 이상이다. 음수로 만들 수 없다. 어기면 `INVALID_STOCK`.

### 행위

| 메서드 | 하는 일 | 거절 |
| --- | --- | --- |
| `Stock(quantity)` | 수량을 검사하고 만든다 | `INVALID_STOCK` |
...
- 0 이상이다. 어기면 `INVALID_PRICE`(이 조각에서 금액이 쓰이는 곳이 가격뿐이라서다. 포인트가 들어오면 금액 자체의 오류 타입으로 나눈다).
